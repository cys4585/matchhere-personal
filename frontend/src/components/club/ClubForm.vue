<template>
  <div class="grid gap-4 sm:gap-6">
    ...
    <div class="form-field">
      <label for="topic">클럽 주제</label>
      <input
        type="text"
        id="topic"
        v-model="fields.topic.value"
        placeholder="ex) 알고리즘, DevOps"
    ...
    <div class="form-field">
      ...
      <textarea
        id="bio"
        v-model="fields.bio.value"
        placeholder="이 클럽에 대해 소개해주세요"
      />
    </div>
    <div class="form-field">
      <label>모집 공개 범위</label>
...
export default {
  ...
  setup(_, { emit }) {
    ...
    const canEdit = computed(() => {
      return fields.coverpic.id && fields.name.value && topicList.value.length
    })
